package com.melodix.app.Utils;

public class Constants {
    public static final String ROLE_USER = "USER";
    public static final String ROLE_ARTIST = "ARTIST";
    public static final String ROLE_ADMIN = "ADMIN";

    public static final String FILTER_ALL = "ALL";
    public static final String FILTER_SONG = "SONG";
    public static final String FILTER_ARTIST = "ARTIST";
    public static final String FILTER_ALBUM = "ALBUM";
    public static final String FILTER_PLAYLIST = "PLAYLIST";

    public static final String SUBMISSION_PENDING = "PENDING";
    public static final String SUBMISSION_APPROVED = "APPROVED";
    public static final String SUBMISSION_REJECTED = "REJECTED";

    public static final String AUDIENCE_ALL = "ALL";
    public static final String AUDIENCE_ARTISTS = "ARTISTS";
    public static final String AUDIENCE_PREMIUM = "PREMIUM";

    public static final String PREFS_APP = "melodix_prefs";
    public static final String PREFS_DATA = "melodix_data";

}
